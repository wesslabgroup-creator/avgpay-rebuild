...
export default function ToolsPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pt-20 pb-14">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <section className="mb-12 text-center">
          <p className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 text-emerald-700 text-sm font-semibold mb-4">
            <Calculator className="w-4 h-4" />
            Salary tools / calculators hub
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 mb-4">Compensation Tools That Help You Decide Faster</h1>
              ...
              <section key={group.intent} className="space-y-4">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-lg bg-white border border-slate-200 flex items-center justify-center">
                    <GroupIcon className="w-5 h-5 text-emerald-600" />
                  </div>
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900">{group.intent}</h2>
                    ...
                    return (
                      <Card key={tool.href} className="border-slate-200 bg-white">
                        <CardHeader className="pb-3">
                          <div className="flex items-center justify-between gap-3">
                            <CardTitle className="text-xl text-slate-900 flex items-center gap-2">
                              <ToolIcon className="w-5 h-5 text-emerald-600" />
                              {tool.name}
                            </CardTitle>
                        ...
                        <CardContent className="space-y-4">
                          <p className="text-slate-700 text-sm">{tool.summary}</p>
                          <p className="text-sm text-slate-600">
                            <span className="font-semibold text-slate-800">Expected outcome:</span> {tool.outcome}
                          </p>

                          <div className="text-sm space-y-2">
                            <p className="font-semibold text-slate-800">Related guides</p>
                            <div className="flex flex-wrap gap-2">
                              {tool.guideLinks.map((guide) => (
                                <Link key={guide.href} href={guide.href} className="px-2.5 py-1 rounded bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors">
                                  {guide.label}
                                </Link>
                              ))}
                            </div>
                          ...
                          <div className="text-sm space-y-2">
                            <p className="font-semibold text-slate-800">Analyzer flows</p>
                            <div className="flex flex-wrap gap-2">
                              {tool.analyzerLinks.map((analyzer) => (
                                <Link
                                  key={analyzer.href}
                                  href={analyzer.href}
                                  className="px-2.5 py-1 rounded bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                                >
                                  {analyzer.label}
                                </Link>
                              ))}
                          ...
                          <Link href={tool.href} className="inline-flex items-center gap-1.5 text-emerald-700 font-semibold hover:text-emerald-800">
                            Open tool
        ...
        <section className="mt-14 bg-white border border-slate-200 rounded-xl p-6 sm:p-8">
          <h2 className="text-2xl font-bold text-slate-900 mb-5">Salary Tools FAQ</h2>
          <div className="space-y-4">
            {FAQS.map((faq) => (
              <div key={faq.question} className="border-b border-slate-100 pb-4 last:border-none last:pb-0">
